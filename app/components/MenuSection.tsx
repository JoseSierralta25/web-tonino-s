...
import { MenuItem, Size, menuData } from '../data/menu';

interface CartItem {
  cartItemId: string; // unique id for the cart item, e.g., `${item.id}-${size}`
  menuItem: MenuItem;
  size?: Size;
  quantity: number;
  subtotal: number;
...
const ProductCard: React.FC<ProductCardProps> = ({ item, onAddToCart }) => {
  const isMultiSize = typeof item.prices === 'object';
  const availableSizes = isMultiSize ? (Object.keys(item.prices as any) as Size[]) : [];
  
  // Default to the first available size, or 'R' if not multi-size
  const [selectedSize, setSelectedSize] = useState<Size | undefined>(
    isMultiSize ? availableSizes[0] : undefined
  );
  const [quantity, setQuantity] = useState(1);

  const price = isMultiSize 
    ? (item.prices as Record<Size, number>)[selectedSize!] 
  ...
  const handleAddToCart = () => {
    ...
    const cartItem: CartItem = {
      cartItemId: selectedSize ? `${item.id}-${selectedSize}` : item.id,
      menuItem: item,
      size: selectedSize,
      quantity,
  ...
  return (
    <div className="bg-[#222222] rounded-xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_30px_rgba(0,0,0,0.5)] flex flex-col relative border border-white/5">
      ...
      {/* Decorative top border representing cheese/sauce */}
      <div className="h-1 w-full bg-gradient-to-r from-[#C62828] via-[#FFC107] to-[#C62828]"></div>

      <div className="p-6 flex-grow flex flex-col">
        <div className="flex justify-between items-start mb-2 gap-2">
          <h3 className="text-xl font-bold text-[#F5F5F0] font-['Sora'] leading-tight">{item.name}</h3>
        </div>
        
        {item.description && (
          <p className="text-[#A0A0A0] text-sm mb-5 leading-relaxed flex-grow">
            {item.description}
          </p>
        )}
        ...
        <div className="mt-auto">
          {/* Size Selector */}
          {isMultiSize && availableSizes.length > 0 && (
            <div className="mb-4">
          ...
          <div className="pt-4 border-t border-white/10">
            <div className="flex justify-between items-end mb-4">
              <div>
                ...
                <p className="text-2xl font-bold text-[#FFC107] font-['Sora']">${price?.toFixed(2) || '0.00'}</p>
              </div>
              ...
              <div className="flex items-center bg-[#1A1A1A] rounded-lg border border-white/10 h-10">
                <button 
                  ...
                  className="w-8 h-full flex items-center justify-center text-[#A0A0A0] hover:text-white hover:bg-white/5 rounded-l-lg transition-colors"
                >
                  -
                </button>
                ...
                <button 
                  ...
                  className="w-8 h-full flex items-center justify-center text-[#A0A0A0] hover:text-white hover:bg-white/5 rounded-r-lg transition-colors"
                >
                  +
                </button>
              </div>
            ...
            <button
              ...
              className="w-full bg-[#C62828] hover:bg-[#b02323] text-white font-bold py-3 px-4 rounded-lg transition-all duration-300 transform active:scale-[0.98] flex items-center justify-center gap-2 uppercase text-sm tracking-wider shadow-lg shadow-red-900/20"
            >
